...
int main()
{
    ...
    {
        if (odleglosc(wspolrzedne.x + wektory[poczatek % wektory.size()].x, wspolrzedne.y + wektory[poczatek % wektory.size()].y) >= odleglosc(wspolrzedne.x, wspolrzedne.y) && ((poczatek - koniec + 1) <= wektory.size()))
        {
