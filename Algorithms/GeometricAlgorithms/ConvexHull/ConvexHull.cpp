...
long long int iloczyn_wektorowy(const punkt &a, const punkt &b, const punkt &c)
{
    ...
    if (x1*y2-x2*y1>0)
    {
        //Punkty zostaly wprowadzone zgodnie z ruchem wskazowek zegara
        return 1;
    }
    if (x1*y2-x2*y1<0)
    {
        //Punkty zostaly wprowadzone przeciwnie do ruchu wskazowek zegara
        return -1;
    }

    //Odcinki powstale z 3 punktow sa do siebie rownolegle
    return 0;
}

void stworz_otoczka()
{
    //Tworzenie gornej czesci otoczki wypuklej 
    for (int i=0;i<punkty.size();i++)
    {
        //Usuwamy z otoczki punkty ktore moze pokryc nowy punkt ktory dodamy
    ...
    //Tworzenie dolnej czesci otoczki
...
int main()
{
    ...
    //Sortujemy punkty po x, nastepnie po y
    sort(punkty.begin(),punkty.end(),cmp);

    //Tworzymy gorna i dolna czesc otoczki wypuklej
    stworz_otoczka();

    //Porownujemy punkty z gornej czesci do dolnej i znajdujemy najodleglejsze punkty
